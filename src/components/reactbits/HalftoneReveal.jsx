...
/**
 * HalftoneReveal — from React Bits (reactbits.dev).
 *
 * Converted from the TypeScript source to JSX to match this
 * codebase, with three changes noted inline:
 *   1. a WebGL2 capability check with an <img> fallback;
 *   2. `alpha: true` so the canvas can sit over the page ground;
 *   3. the RAF loop pauses when the canvas is off screen.
 * Everything else — the shader, the uniforms, the maths — is
...
/** The shaders are `#version 300 es`, which is WebGL2 only. */
...
const HalftoneReveal = ({
  ...
  // Decided once on mount rather than at module scope, so this stays
  // safe if the app is ever server-rendered.
  ...
  useEffect(() => {
    ...
    const renderer = new Renderer({
      ...
      // Upstream uses alpha:false. Here the canvas has to sit over
      // the page ground and be masked at its edges, so it needs a
      // real alpha channel.
      alpha: true,
      antialias: true
    });
    ...
    // Pointer listeners go on the nearest section rather than the
    // canvas: the container is pointer-events:none in the hero so it
    // can't swallow clicks on the copy above it.
    const surface = container.closest('[data-halftone-surface]') ?? container;
    ...
    surface.addEventListener('pointerenter', onMove, { passive: true });
    surface.addEventListener('pointerleave', onLeave, { passive: true });

    // A full-screen fragment shader this heavy has no business
    // running while it's scrolled out of view.
    let visible = true;
  ...
  return (
    <div
      ref={containerRef}
      className={`relative w-full h-full overflow-hidden touch-none ${className}`.trim()}
      style={{ borderRadius, ...style }}
    />
  );
};
