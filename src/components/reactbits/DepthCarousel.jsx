...
/**
 * DepthCarousel — from React Bits (reactbits.dev).
 *
 * Converted from TypeScript to JSX. Logic is upstream's; the only
 * additions are `onChange` firing on mount so a detail panel beside
 * it starts in sync, and a `renderOverlay` slot for per-card
 * content.
 */
...
const DepthCarousel = ({
  ...
  // Upstream only fires onChange on a *change*, so a detail panel
  // rendered alongside would start empty until the first navigation.
  useEffect(() => {
    if (count > 0) onChangeRef.current?.(0, data[0]);
  }, [count, data]);
